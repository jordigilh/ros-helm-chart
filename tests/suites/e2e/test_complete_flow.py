    ...
    def test_04_manifest_created_in_koku(self, cluster_config, registered_source):
        ...
        def check_manifest():
            try:
                ...
                # Transient DB connection issues - retry on next poll
                return False
        
        success = wait_for_condition(
            check_manifest,
            timeout=600,  # 10 minutes (increased from 5 to handle timing variations)
            interval=10,   # Poll every 10 seconds (decreased from 15 for more responsive checking)
