...
1. **gcsweb URL** (easiest - copy from GitHub CI check "Details" link):
   ```
...
```bash
# From gcsweb URL (copy from GitHub)
...
gcloud storage cp \
  "gs://test-platform-results/pr-logs/pull/insights-onprem_cost-onprem-chart/<PR>/pull-ci-insights-onprem-cost-onprem-chart-main-e2e/<BUILD_ID>/artifacts/e2e/insights-onprem-cost-onprem-chart-e2e/artifacts/junit*.xml" \
  ./
```
...
```bash
# Build log (main CI operator log)
...
| `artifacts/e2e/insights-onprem-cost-onprem-chart-e2e/artifacts/junit.xml` | JUnit test report |
