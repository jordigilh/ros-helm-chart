...
class DataUploadPhase:
    ...
    def cleanup_ocp_processing_artifacts(self) -> Dict[str, int]:
        ...
        1. Old Parquet files from S3 (data/parquet/*/OCP/)
        ...
        # 1. Delete Parquet files from S3
        parquet_prefixes = [
        ...
        if self.k8s:
            ...
            for table in trino_tables:
                try:
                    ...
                    result = self.k8s.execute_command_in_pod(
                        pod_name='trino-coordinator-0',
                        namespace='cost-mgmt',
    ...
    def upload_ocp_format(self,
        ...
        # CRITICAL: Cleanup old Parquet files and Trino tables for clean test state
        if force:
            print(f"\n🧹 Cleaning old Parquet files and Trino tables...")
            ...
            if cleanup_result['parquet_deleted'] > 0 or cleanup_result['tables_dropped'] > 0:
                ...
                print(f"     - Parquet files deleted: {cleanup_result['parquet_deleted']}")
                print(f"     - Trino tables dropped: {cleanup_result['tables_dropped']}")
            else:
                print(f"  ℹ️  No old artifacts found")
        ...
        if smoke_test:
            ...
            # Use static report file that generates ONLY pod_usage
