...
create_koku_db_secret() {
    ...
    echo_info "  Secret name: $secret_name (referenced in values-koku.yaml)"
}
...
validate_chart_structure() {
    ...
    if [ -z "$VALUES_FILE" ]; then
        # Look for default values files
        if [ -f "$chart_dir/values-koku.yaml" ]; then
            VALUES_FILE="$chart_dir/values-koku.yaml"
            echo_info "Using default values file: values-koku.yaml"
