...
# Note: Admin credentials are auto-generated by RHBK operator (rhsso-keycloak-initial-admin secret)
...
# The operator automatically creates rhsso-keycloak-initial-admin secret when Keycloak instance is deployed
...
EOF
    ...
    local ADMIN_PASSWORD=$(oc get secret rhsso-keycloak-initial-admin -n "$NAMESPACE" -o jsonpath='{.data.password}' 2>/dev/null | base64 -d)
    ...
    while [ $post_import_elapsed -lt $post_import_timeout ]; do
        ...
        if [ -n "$access_token" ]; then
            # Try to get the client we just created
            local client_data=$(curl -sk -X GET "$KEYCLOAK_URL/admin/realms/$REALM_NAME/clients" \
                -H "Authorization: Bearer $access_token" 2>/dev/null)

            # Check if our client is in the list
            if echo "$client_data" | grep -q "\"clientId\":\"$COST_MGMT_CLIENT_ID\""; then
                echo_success "✓ Client '$COST_MGMT_CLIENT_ID' is available via admin API"
...
extract_client_secret() {
    ...
    if [ -z "$ADMIN_PASSWORD" ]; then
        echo_error "Could not retrieve auto-generated admin password from rhsso-keycloak-initial-admin secret"
...
display_summary() {
    ...
    echo_info "  oc get secret rhsso-keycloak-initial-admin -n $NAMESPACE -o jsonpath='{.data.username}' | base64 -d"
    echo_info "  oc get secret rhsso-keycloak-initial-admin -n $NAMESPACE -o jsonpath='{.data.password}' | base64 -d"
    echo ""
...
cleanup_deployment() {
    ...
    # Remove secrets
    # Note: rhsso-keycloak-initial-admin is managed by operator and will be auto-deleted
...
case "${1:-}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo "      Access them via: oc get secret rhsso-keycloak-initial-admin -n keycloak"
        echo ""
        echo "Examples:"
